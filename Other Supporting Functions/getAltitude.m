...
% r.accel               - Accelration info
% r.t                   - Time stamps for acceleration info
    ...
    while any(flying,"all") % When the rocket reaches apogee, stop simulating
        ...
        F = D + T - m.*g;
        a = F./m.*flying; % Only update if you're still flying

        % Kinematics
        % - Updates the rocket's position and altitude.
        % - Updates mission elapsed time
        v = a.*dt.*flying + v; % Only update if you're still flying
        ...
        % Log Time vs. Acceleration for Comparison to Real Flight Curves
        accel_log(round(t/dt),:,:) = a;
        time_log(round(t/dt),:,:)  = t*ones(size(a));
        alt_log(round(t/dt),:,:)   = h;
        vel_log(round(t/dt),:,:)   = v;
